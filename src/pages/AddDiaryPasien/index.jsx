...
const AddDiaryPasien = () => {
  const location = useLocation();
  const { data } = location.state || {}; // Mendapatkan data dari lokasi (state) jika ada
  const { dataUser } = location.state || {}; // Mendapatkan data dari lokasi (state) jika ada
    ...
    try {
      const diaryData = {
        data: {
          ...
          users_permissions_user: dataUser,
          doctor: auth()?.id,
        },
      };
      ...
      navigate("/consultant"); // Redirect setelah sukses
    } catch (error) {
